...
// **************************[Error messages]******************************//
#define ERRORNOSERIALDATA 0 // STK500_recv returned less than 0
...
class STK500
{
	...
	protected:
		...
		virtual int readPage(avrmem *buf) = 0;
		void setup();
		...
		int STK500_loadaddr(unsigned int addr);
		int STK500_paged_write(AVRMEM * m, int page_size, int n_bytes);
		int STK500_program_enable();
		void STK500_disable();
		
		unsigned char linebuffer[HEXLINELENGTH];
